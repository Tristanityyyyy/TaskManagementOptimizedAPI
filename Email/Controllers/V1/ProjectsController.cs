...
public sealed class ProjectsController : ApiControllerBase
{
    ...
    public async Task<ActionResult<PagedResult<ProjectListItemResponse>>> List(
        ...
        CancellationToken cancellationToken = default)
        => Ok(await _projects.ListAsync(CurrentAccount.Id, page, pageSize, includeDeleted, cancellationToken));
    ...
    public async Task<ActionResult<ProjectListItemResponse>> Get(int id,
        CancellationToken cancellationToken = default)
    {
    ...
    [HttpPost]
    [ProducesResponseType(typeof(ProjectListItemResponse), StatusCodes.Status201Created)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<ActionResult<ProjectListItemResponse>> Create(
    ...
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<ActionResult<ProjectListItemResponse>> Update(int id,
        [FromBody] UpdateProjectRequest dto,
        CancellationToken cancellationToken = default)
        => Ok(await _projects.UpdateAsync(CurrentAccount.Id, id, dto, cancellationToken));
    ...
    {
        await _projects.RestoreAsync(CurrentAccount.Id, id, cancellationToken);
        return NoContent();
    }
}
